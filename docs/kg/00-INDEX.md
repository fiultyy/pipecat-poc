...
| N11 | [11-console-observation-p1.md](11-console-observation-p1.md) | **Console 观测面 P1 ✅topic 数据平面**：head.turn/fleet.snapshot/bridge.msg/tickets.snapshot 四源 + FileTailer（行尾/快照）+ TurnTrace（帧→phase）+ observe 会话/topics 订阅协议 + `fleet.cleanup`/`fleet.cleanup.result` 席位清理控制帧（release=只出册/end=loopback 真死会话 workspace.archiveSession+出册、逐 id ok/error、active-liaison note、fleet.json 只原子写）（P1.5 cmd 通用路由、P2 SPA 待开） |
...
| N14 | [14-unified-callback-split.md](14-unified-callback-split.md) | **回调分流统一框架（A/B/C/D 合并裁决稿，PR1–PR5 ✅收口，PR7 工具面扩展）**：rt_session_store（key=ref+SQLite 落盘）、orch.done 载 body 直载台账（`_store_bridge` 唯一写入面）、body.push 单 topic+body.get 帧、VOICE_FINAL_MODE=fulltext\|split 终稿分流（缺省 split，[编排通报] 纯数据 JSON+store 降级，精确 fulltext 回退）、VOICE_RECEIPT_SLIM 回执瘦身、工具面十二件套（read_body ref_or_no/max_chars/from_tail+ok/miss/error 三形态+C 降级、list_bodies 索引、cancel_run ref-only、工具只读台账；PR7 工作区文件五件套 find_files/grep_files/read_file/edit_file/write_file：workspace_root=VOICE_WORKSPACE/仓库根、越界拒绝、量限封顶、垃圾目录二进制剪除、doctrine 读取不出声+改完一句话）、notify 相（head.turn phase=notify 含 pending 补投）、详情页并「任务」页签（台账+正文+tickets 全文上下 Paned，bridge 行归编排页）+语音页迷你通知行+回合页 📣/「└已入详情」+席位清理控制（fleet_tree 多选+释放/结束双按钮+tk confirm 确认门→fleet.cleanup，规格 N11）、head.compact 零 LLM 压缩（llm.py mirror_sink/on_turn_idle tap+delete_conversation_item、events.py item.delete 上游已备零改动、删非 pinned+state.snapshot 单 item、VOICE_COMPACT_CHARS 阈值、_final_inject_lock 锁序）——五面（store/观测/工具/显示/压缩）全部落地 |
| N15 | [15-head-profile-registry.md](15-head-profile-registry.md) | **head 配置注册表（PR8 ✅：多 head 配置，激活单例，echo/scribe 副本占位）**：rt_head_registry（profiles JSON=VOICE_HEAD_PROFILES/\~/.config/voice-gateway/heads.json，逐字段 profile→旧 env/内置回落、全 null 条目=旧单头精确副本、坏文件整体降级单头不炸）、build 按激活 profile 取 model/voice/doctrine/VAD、head.list/head.switch 控制帧（切换写回文件、下一次语音连接生效、活会话不拆、VOICE_HEAD_PROFILE 钉住拒绝）、语音页 head 单选行、多 head 并发激活留待裁决 |
