...
exposes the ``search_web_pages`` and ``fetch_page_content`` tools of a hosted
MCP server powered by Keenable AI (https://keenable.ai). Pass
``await search.tools()`` into the ``LLMContext`` and the LLM auto-registers the
tools' handlers, so the agent can answer questions about current events and
...
You have two tools:
- search_web_pages: search the web for current events, news, or any facts that may be beyond your training data or need to be up to date.
- fetch_page_content: read the text of a specific web page when the user gives you a URL.

Prefer these tools over guessing or relying on memory whenever a question needs current information. When the user gives you a URL, read it with fetch_page_content even if you think you already know its contents.
